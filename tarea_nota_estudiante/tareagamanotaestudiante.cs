...
namespace tareagama
{
    public partial class Form1 : Form
    {
        ...
        private void btnCalcular_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Por favor ingresa las 4 notas válidas (0-100).", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        private void btnCompletivo_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Por favor ingresa una nota de completivo válida (0-100).", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (!double.TryParse(lblPromedio.Text, out double promedio))
            {
                MessageBox.Show("Primero calcula el promedio, por favor.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                lblResultado.Text = "No alcanzaste la nota mínima (tras completivo: " + notaCompleta.ToString("0.##") + ").";
                // permitir extraordinario
                txtExtraordinario.Enabled = true;
                btnExtra.Enabled = true;
                lblResultado.Text += "\nSi quieres, puedes intentar el extraordinario.";
            }
            else
            {
        ...
        private void btnExtra_Click(object sender, EventArgs e)
        {
            if (!double.TryParse(txtExtraordinario.Text, out double extra)
                || extra < 0 || extra > 100)
            {
                MessageBox.Show("Por favor ingresa una nota de extraordinario válida (0-100).", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (!double.TryParse(txtCompletivo.Text, out double completivo))
            {
                MessageBox.Show("Ingresa la nota de completivo antes de intentar el extraordinario, por favor.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (notaExtra <= 69.0)
                lblResultado.Text = "Sigue sin alcanzar la nota mínima (resultado extraordinario: " + notaExtra.ToString("0.##") + ")";
            else
                lblResultado.Text = "¡Felicidades! Aprobaste por extraordinario (nota: " + notaExtra.ToString("0.##") + ")";
        }
